  ...
  <div class="max-w-3xl mx-auto p-8 text-MyWhite font-main">
    ...
    <div v-if="cart.items.length">
      <div v-for="item in cart.items" :key="item.id"
        ...
        <img v-if="item.coverImage" :src="item.coverImage" alt="Cover" class="w-20 h-20 object-cover rounded shadow" />
        ...
        <div class="flex-1">
          ...
          <div class="flex items-center mt-2">
            <QuantitySelector v-model="item.quantity" :min="1" @update:modelValue="val => updateQuantity(item, val)" />
          </div>
        </div>

        <TrashButton title="Remove from cart" alt="Remove" class="flex items-center" :disabled="false"
